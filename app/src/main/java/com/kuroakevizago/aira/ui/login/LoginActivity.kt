...
import com.kuroakevizago.aira.ui.signup.SignupActivity
        ...
        viewModel.loginStatus.observe(this) { result ->
            when (result) {
                ...
                is ResultStatus.Success -> {
                    showLoading(false)
                    viewModel.saveSession(UserModel(email, result.data.data?.token!!))
                    moveToMainActivity()
                }
            }
        }
                ...
                if (task.isSuccessful) {
                    ...
                    user?.getIdToken(true)?.addOnCompleteListener {
                            if (it.isSuccessful) {
                                ...
                                viewModel.saveSession(UserModel(user.email!!, token!!))
                                moveToMainActivity()
                            } else {
